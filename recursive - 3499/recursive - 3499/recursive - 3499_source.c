...
int main(void)
{
    ...
    {
        memmove(secondarr - ((N / 2)+1), secondarr, sizeof(char) * (N/2));
    }
    mix(N);
...
void mix(int N)
{
    ...
    else
    {
        if (N % 2 == 0)
        {
          /*  resultarr[N - 1] = secondarr[(N/2) - 1]; */// 짝수
            resultarr[N - 1] = secondarr[(N / 2) + 3]; // 홀수
        }
        else
        {
    ...
    printf("N:%d, firstarr:%s\n",N,firstarr);
    printf("N:%d, secondarr:%s\n",N, secondarr);
    mix(N - 1);
}
